...
The `docs/figures/*.png` charts committed in this repository were generated from
the **superseded raw-unit run**: they contradict every number above and must not
be shown with this deck. They are retained for provenance alongside
[`Findings.md`](Findings.md), which is the document they illustrate.
