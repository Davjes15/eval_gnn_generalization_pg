...
> The repository has **stacked branches** (`step-1-…` → `step-8-…`) plus `main`.
> Each step branch builds on the previous one. **You only need one branch to run
> everything: `step-8-regime-comparison`** — it is the top of the stack and
> contains *all* the code (data generation, the 6 models, the experiment driver,
> the frozen configurations, normalization, the validation gates, the
> checkpoint-replay tooling, and the committed grid files). `main` holds
> documentation, figures and saved results — not the runnable pipeline.
>
...
git fetch origin
git checkout step-8-regime-comparison
```
...
git clone https://github.com/Davjes15/eval_gnn_generalization_pg.git
git clone https://github.com/PowerGraph-Datasets/PowerGraph-Node.git
cd C:\gnn\eval_gnn_generalization_pg; git fetch origin; git checkout step-8-regime-comparison
